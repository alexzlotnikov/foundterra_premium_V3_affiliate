...
export function FoundterraCard({ deal }: { deal: Deal }) {
  ...
  return (
    <a href={deal.link} className="block rounded-2xl border border-[#6366F1] bg-[linear-gradient(135deg,#1E1B4B_0%,#0F1A2E_100%)] p-6 transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_8px_32px_rgba(99,102,241,0.2)]">
      ...
      <div className="inline-flex items-center gap-2 rounded-md bg-[#2b2f92] px-4 py-2 text-sm font-semibold text-white">{isTools ? 'Get Free Tools' : 'Book $100 Pitch Review'} <ArrowRight size={15} /></div>
    </a>
  );
}
